...
void makeQ1b(const float T_in = 0.3)
{
  gROOT->Reset();
  ...
  const Char_t *FileName[N] = {"../data/PHSD/charm/homework1b-charm-phsd.dat",
			       "../data/CUJET/LGR/LGR_Q1/Q1_c_EtaD.dat",
			       "../data/Duke/normalized_drag.dat",
			       "../data/TAMU/Z_results-data/1b_norm_A.txt",
  ...
  if(iT==2) {  // Nantes T = 0.2 GeV
    gr_add[0] = new TGraph("../data/Nantes/task1/norm_drag_0.2.txt","%lg %lg");
  }
  if(iT==7) {  // Nantes T = 0.3 GeV
    gr_add[0] = new TGraph("../data/Nantes/task1/norm_drag_0.3.txt","%lg %lg");
  }
  if(iT==12) {  // Nantes T = 0.4 GeV
    gr_add[0] = new TGraph("../data/Nantes/task1/norm_drag_0.4.txt","%lg %lg");
  }
  ...
  TLegend *leg = new TLegend(0.68, 0.88-(N+N_add_exit)*0.06, 0.94, 0.94);
  leg->SetFillColor(10);
  //   leg->SetFillStyle(10);
  ...
  for(int i=0;i<N;i++) leg->AddEntry(gr[i][iT],Name[i],"l");
  for(int i=0;i<N_add;i++) {
  ...
  c1->Update();
  c1->SaveAs(Form("../fig/Q1b_A_%4.2f.pdf",T_in));
  c1->SaveAs(Form("../fig/Q1b_A_%4.2f.png",T_in));
