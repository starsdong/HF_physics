...
void makeQ1a_bottom()
{
  ...
  const Int_t N = 9;
  const Int_t Ng = 3;
  Char_t *FileName[N] = {"../data/PHSD/bottom/homework1a-phsd.dat",
			 "",
			 "",
			 "",
			 "../data/Catania/Dx_Bottom.dat",
			 "",
			 "",
			 "",
			 "../data/DAB-MOD/Ds2PiT_bottom.txt"};
  const Char_t *Name[N] = {"PHSD","CUJET","Duke","TAMU","Catania","LBT","Nantes","Nantes el.","DAB-MOD"};
  const Int_t Color[N] = {kBlack, kRed, kGreen+2, kBlue, kOrange+1, kGray+2, kMagenta+1, kMagenta+1, kAzure+1};
  const Int_t Style[N] = {1, 2, 3, 9, 5, 6, 7, 8, 4};
  TGraph *gr[N];
  for(int i=0;i<N;i++) {
    if(i==1 || i==2 || i==3 || i==5 || i==6 || i==7) continue;
  ...
  TLegend *leg = new TLegend(0.18, 0.94-Ng*0.06, 0.46, 0.94);
  leg->SetFillColor(10);
  //   leg->SetFillStyle(10);
  ...
  c1->Update();
  c1->SaveAs("../fig/Q1a_bottom_2piTDs.pdf");
  c1->SaveAs("../fig/Q1a_bottom_2piTDs.png"); 
