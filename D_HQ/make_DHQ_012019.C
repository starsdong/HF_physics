...
void make_DHQ_012019()
{
  ...
  const Double_t x_txt1[n_txt1] = {0.75, 0.63, 2.0, 1.7, 1.6, 1.6, 2.1, 2.02, 2.0, 1.6, 2.25, 2.1};
  const Double_t y_txt1[n_txt1] = {14.5, 11.8, 32.5, 32.2, 21, 7.6, 10., 12.5, 7., 2.5, 2.8, 5.5};
  const Double_t angle_txt1[n_txt1] = {-48, -50., 5, 30, 7, 15, 14, 15, 12, 4, 0., 8.0};
  ...
  TLatex *tex = new TLatex(0.6, 34.2, "Lattice QCD");
  tex->SetTextFont(42);
  tex->SetTextSize(0.04);
  tex->Draw("same");
  
  TLegend *leg = new TLegend(0.16, 0.74, 0.3, 0.88);
   leg->SetFillColor(10);
   //   leg->SetFillStyle(10);
  ...
  */
  ...
  c1->Update();
  c1->SaveAs("fig/D_HQ_012019.eps");
  c1->SaveAs("fig/D_HQ_012019.pdf");
  c1->SaveAs("fig/D_HQ_012019.png");
}
