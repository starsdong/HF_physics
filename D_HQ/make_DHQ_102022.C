...
void make_DHQ_102022(int config=0)
{
  //  gROOT->Reset();
  style();
  ...
  const Double_t x_txt1[n_txt1] = {0.7, 0.6, 2.0, 1.7, 1.6, 1.6, 2.1, 2.02, 2.0, 1.6, 2.25, 2.1};
  const Double_t y_txt1[n_txt1] = {16.5, 13, 32.5, 32.2, 21, 7.6, 10., 12.5, 6.5, 2.5, 2.8, 5.5};
  //  const Double_t angle_txt1[n_txt1] = {-40, -45., 5, 30, 7, 12, 13, 14, 10, 4, 0., 7.};
  const Double_t angle_txt1[n_txt1] = {-65, -68., 5, 30, 7, 12, 13, 14, 10, 4, 0., 7.};
  ...
  for(int i=0;i<n_L_Ding;i++) {
    ...
    xe_L_Ding[i] = 0.01;
    yeL_L_Ding[i] = sqrt(pow(ye_L_Ding[i],2.)+pow(yesL_L_Ding[i],2.));
    yeU_L_Ding[i] = sqrt(pow(ye_L_Ding[i],2.)+pow(yesU_L_Ding[i],2.));
  }
  inData.close();
  ...
  for(int i=0;i<n_L_Banerjee;i++) {
    ...
    yeL_L_Banerjee[i] = sqrt(pow(ye_L_Banerjee[i],2.)+pow(yesL_L_Banerjee[i],2.));
    yeU_L_Banerjee[i] = sqrt(pow(ye_L_Banerjee[i],2.)+pow(yesU_L_Banerjee[i],2.));
  }
  inData.close();
  ...
  TGraphErrors *gr_L_HotQCD = new TGraphErrors("/Users/starsdong/work/datapoints/D_HQ/LQCD_HotQCD.txt","%lg %lg %lg");
  ...
  gr_L_HotQCD->SetLineWidth(2);
  gr_L_HotQCD->SetLineColor(1);
  gr_L_HotQCD->SetMarkerSize(1.2);
  gr_L_HotQCD->SetMarkerStyle(20);
  gr_L_HotQCD->SetMarkerColor(1);
  gr_L_HotQCD->Draw("p");
  ...
  TLatex *tex;
  if(config==0) tex = new TLatex(0.8, 45, "Lattice QCD");
  else if(config==1) tex = new TLatex(0.8, y2*0.9, "Lattice QCD");
  tex->SetTextFont(42);
  tex->SetTextSize(0.04);
  tex->Draw("same");
  
  TLegend *leg = new TLegend(0.26, 0.7, 0.4, 0.88);
   leg->SetFillColor(10);
   //   leg->SetFillStyle(10);
   ...
   leg->AddEntry(gr_L_HotQCD,"HotQCD","p");
   leg->Draw();
  ...
  */
  ...
  c1->Update();
  c1->SaveAs("fig/D_HQ_102022.pdf");
  c1->SaveAs("fig/D_HQ_102022.png");
}
