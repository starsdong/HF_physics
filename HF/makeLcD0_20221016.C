...
void makeLcD0_20221016(const Int_t mPlotTh = 0)
{
//  gROOT->Reset();
  style();
  ...
  fin = new TFile("root/PYTHIA8_D_ratio_200GeV_20191101_CR.root");
  ...
  tex = new TLatex(1, 0.78, "Ko: three-quark");
  tex->SetTextFont(12);
  tex->SetTextSize(0.06);
  tex->Draw("same");

  tex = new TLatex(2, 1.3, "Ko: di-quark");
  tex->SetTextFont(12);
  tex->SetTextSize(0.06);
  tex->Draw("same");
  ...
  for(int i=0;i<NConfig;i++) {
   ...
   leg = new TLegend(0.54, 0.68, 0.95, 0.78);
   leg->SetFillColor(10);
   leg->SetFillStyle(10);
   leg->SetLineStyle(4000);
   leg->SetLineColor(10);
  ...
  c1->SaveAs("fig/LcD0Ratio_20221016.pdf");
  c1->SaveAs("fig/LcD0Ratio_20221016.png");
