...
void makeQ2a_Lc()
{
  ...
  Char_t *FileName[N] = {"",
			 "",
			 "../data/TAMU/Z_results-data/2a_HAA_Lc_1.txt",
			 "../data/TAMU/Z_results-data/2a_HAA_Lc.txt",
			 "../data/Catania/Lc_ratio.dat",
			 "../data/LBT/LBT/data-2a-HAA-LBT-Lambda-c.dat",
			 "",
			 "",
			 "",
			 ""};
  const Char_t *Name[N] = {"PHSD","CUJET","TAMU-1","TAMU-2","Catania","LBT","Nantes","Nantes el.","DAB-MOD","LANL"};
  const Int_t Color[N] = {kBlack, kRed, kBlue, kBlue, kOrange+1, kGray+2, kMagenta+1, kMagenta+1, kAzure+1, kBlack};
  const Int_t Style[N] = {1, 2, 3, 9, 5, 6, 7, 8, 4, 10};

  TGraph *gr[N];
  for(int i=0;i<N;i++) {
    if (i==0 || i==1 || i==6 || i==7 || i==8 || i==9) continue;
  ...
  h0->GetYaxis()->SetTitle("H_{AA} = R_{AA}^{H}/R_{AA}^{Q}");
  ...
  c1->Update();
  c1->SaveAs("../fig/Q2a_Lc_HAA.pdf");
  c1->SaveAs("../fig/Q2a_Lc_HAA.png"); 
