...
void makeDsD0_data(const Int_t mPlotTh = 0)
{
  gROOT->Reset();

  const Int_t NConfig = 3;
  const Int_t NMax = 4;
  const Int_t N[NConfig] = {4, 4, 3}; // ALICE_pp, ALICE_PbPb, STAR_AuAu
  const Char_t *ExpName[NConfig] = {"ALICE","ALICE","STAR"};
  const Char_t *CollName[NConfig] = {"pp_7TeV","PbPb_30_50_5p02TeV","AuAu_10_40_200GeV"};
...
//  gr_pythia8->Draw("c");
  ...
  for(int i=0;i<NConfig;i++) {
   ...
   //   leg->AddEntry(gr_pythia8, " PYTHIA8 @7TeV", "l");
   leg->Draw();

   TLatex *tex = new TLatex(6.5, 0.52, "STAR Preliminary");
   tex->SetTextFont(32);
   tex->SetTextColor(2);
  ...
  c1->SaveAs("fig/DsD0Ratio_data_190313.pdf");
  c1->SaveAs("fig/DsD0Ratio_data_190313.png");
